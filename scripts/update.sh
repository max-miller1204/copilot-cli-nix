...
readonly GITHUB_REPO="github/copilot-cli"
...
fetch_native_hash() {
    local version="$1"
    local platform="$2"
    local url="${GITHUB_RELEASE_BASE}/v${version}/copilot-${platform}.tar.gz"
...
main() {
    ...
    log_info "Successfully updated copilot from $current_version to $latest_version"
